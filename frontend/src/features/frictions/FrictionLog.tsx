...
export function FrictionLog({ limit = 20, resolved }: Props) {
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-mist">記録された詰まりはありません</p>;
  }
          ...
          <div className="flex shrink-0 items-center gap-1">
            {!f.resolved_at && (
              <button
                ...
                className="inline-flex h-6 items-center justify-center rounded px-2 text-xs text-mist transition-colors hover:text-growth"
                aria-label="解決にする"
              >
                解決
              </button>
            ...
            <button
              ...
              className="inline-flex h-6 w-6 items-center justify-center rounded text-base text-mist transition-colors hover:text-danger"
              aria-label="削除"
            >
              ×
            </button>
